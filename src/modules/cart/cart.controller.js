...
const add = asyncHandler(async (req, res) => {
    ...
    if (!productId || !qty) throw apiError(400, "Product ID and quantity are required");

    const result = await CartService.addToCart(req.user._id, productId, qty);

    res.status(OK).json(apiResponse(OK, result, "Product added to cart successfully"));
...
const updateQty = asyncHandler(async (req, res) => {
    ...
    if (!productId || qty === undefined) throw apiError(400, "Product ID and quantity are required");

    const result = await CartService.setQty(req.user._id, productId, qty);

    res.status(OK).json(apiResponse(OK, result, "Cart quantity updated successfully"));
});
...
const remove = asyncHandler(async (req, res) => {
    ...
    res.status(OK).json(apiResponse(OK, result, "Product removed from cart successfully"));
});
